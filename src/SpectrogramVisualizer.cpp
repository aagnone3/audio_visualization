...
void SpectrogramVisualizer::plotSpectrogram() {
  ...
  glMatrixMode(GL_MODELVIEW);
  glPushMatrix();
  glTranslatef(x0+(AudioInput::N_TIME_WINDOWS-runTime/secondsPerPixel)/viewportSize[0], y0, 0);
  glScalef(1.0F/(viewportSize[0]*secondsPerPixel), 1.0F/(viewportSize[1]*hzPerPixel), 1);

  /* plot axes */
  char xLabel[] = "t(s)", yLabel[] = "f(Hz)";
  drawAxes(runTime-endTime, runTime, EPSILON, highestFrequency, 2.0, 2.0, xLabel, yLabel);
